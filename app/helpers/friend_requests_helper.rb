module FriendRequestsHelper
	...
	def relationship_with(user)
		if friend_request_pending?(user)
			return '<span class="label label-warning" style="position:relative;font-size:12px;top:20px;left:10px;height:22px;">Pending friend request</span>'
		elsif have_to_respond_to_friend_request(user)
			return '<span class="label label-important" style="position:relative;font-size:12px;top:20px;left:10px;height:22px;">Respond to friend request</span>'
		elsif friends?(user)
			return '<span class="label label-success" style="position:relative;font-size:12px;top:20px;left:10px;height:22px;">Friends!</span>'
		else 
