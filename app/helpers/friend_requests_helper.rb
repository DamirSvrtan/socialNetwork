module FriendRequestsHelper
	...
	def relationship_with(user)
		if friend_request_pending?(user)
			return '<span class="label label-warning" style="position:relative;font-size:12px;top:2px;height:20px;">Pending friend request</span>'
		elsif have_to_respond_to_friend_request(user)
			return '<span class="label label-important" style="position:relative;font-size:12px;top:2px;height:20px;">Respond to friend request</span>'
		elsif friends?(user)
			return '<span class="label label-success" style="position:relative;font-size:12px;top:2px;height:20px;">Friends</span>'
		else 
